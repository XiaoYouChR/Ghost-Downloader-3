...
class MobileTaskPage(TaskPage):
    ...
    def _initWidget(self):
        ...
        # 换图标化 ToolButton(PushButton 清文字后图标渲染不全); 须在基类 _bind 前替换
        for old in (self.allStartButton, self.allPauseButton):
            old.hide()  # deleteLater 异步, 不先 hide 旧按钮会暂留 (0,0) 被绘制
            old.deleteLater()
    ...
    def _mobileCardClass(self, task: Task) -> type:
        ...
        taskType = type(task)
        cached = self._cardClassCache.get(taskType)
