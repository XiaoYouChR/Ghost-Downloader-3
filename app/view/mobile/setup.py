...
def setupSystemFont() -> None:
    ...
    # 各 OEM 系统默认字体: MIUI/HyperOS、ColorOS、HarmonyOS、OriginOS。Qt 字体库已收录则直接用, 否则加载文件取族名
    candidates = [
...
def setupFluentIconRendering() -> None:
    """FluentIcon→QIcon 改走 QSvgRenderer, 绕开 Android 不可用的 qsvg 插件(W^X 下 dlopen 不了 → 图标全空白)。

    须同时 patch FluentIconBase.icon 和 Icon: 菜单经 Icon(=QIcon(svg 路径))持图标, 无 qsvg 时 isNull()=True 致整列无图标。
    """
...
def setupTouchScrolling(window) -> None:
    """给滚动区抓 QScroller 触摸手势 + 装「滑动吞点击」守卫 —— Qt Widgets 在 Android 默认无惯性甩动。

    跳过 ExpandSettingCard(它应随页面整体滚动, 不内部独立滚)。守卫: 滑动时 Qt 仍向手指下控件合成 mouse release 会误触, 吞掉。
    """
