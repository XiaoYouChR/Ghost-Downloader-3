...
# GitHub's own 16px octicon paths, one per alert kind. fill is set at render time so the icon
# colour matches the alert's theme colour (single source of truth in theme.alertColors).
_OCTICONS = {
    "note": (
...
def toAlertIcon(kind: str, color: str, size: int = 16) -> QPixmap:
    svg = (
    ...
    # Render into the logical size rect so the painter's dpr transform maps it onto the full
    # device pixmap — without an explicit rect this double-scales and clips on high-dpi screens.
