"""APK 瘦身 —— 裁掉纯 QWidget app 用不到的 Qt 模块。

PySide6 wheel 把全套 Qt(~135 模块)原样 bundle, 且顶层 lib/ 与 bundle 内各存一份(重复 ~129MB)。
用 allowlist 双删去重: 未在 KEEP 的模块一律删, Qt 新增模块也自动裁掉; 误删会在真机启动暴露, 补进 KEEP 即可。
"""
...
# Multimedia/MultimediaWidgets 不在 KEEP(app 没碰), 连带删 Qt 自带 ffmpeg 后端(~16MB);
# 媒体合流用 gd3ffmpeg 的独立二进制, 与 Qt Multimedia 无关。
