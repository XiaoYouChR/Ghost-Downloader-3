...
# 主线程预热 nativeLibraryDir 的 lru_cache: pyjnius autoclass 在后台线程取不到 classloader,
# 而 ffmpegPaths() 等会在 coreService 后台事件循环里被调; 先缓存住路径, 后台再调即不碰 jnius。
from app.supports.android import nativeLibraryDir as _warmNativeLibDir
_warmNativeLibDir()
