...
fun isProxyHostValid(host: String): Boolean {
    val value = host.trim()
...
fun isProxyValid(input: ProxyInput): Boolean = input.mode != ProxyMode.CUSTOM || (
    input.protocol in listOf("http", "https", "socks4", "socks5", "socks5h") &&
    isProxyHostValid(input.host) && input.port.toIntOrNull() in 1..65535 &&
    isProxyCredentialsValid(input)
)

fun isProxyCredentialsValid(input: ProxyInput): Boolean =
...
fun ProxyForm(initial: String, state: SettingsEditState, onSave: (String) -> Unit, onBack: () -> Unit,
    ...
    SettingsEditor(stringResource(R.string.proxy_edit), state, input != parseProxy(initial), isProxyValid(input),
        onSave = { onSave(buildProxy(input)) }, onBack = onBack, modifier = modifier,
        onInvalid = {
            shouldValidate = true
            when {
                !isProxyHostValid(input.host) -> hostFocus.requestFocus()
        ...
        Column(Modifier.padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            if (isCustom) {
                ...
                OutlinedTextField(input.host, { input = input.copy(host = it) }, singleLine = true,
                    ...
                    isError = shouldValidate && !isProxyHostValid(input.host),
                    supportingText = { Text(stringResource(if (shouldValidate && !isProxyHostValid(input.host))
                ...
                if (isAuthExpanded) {
                    ...
                    OutlinedTextField(input.password, { input = input.copy(password = it) }, singleLine = true,
                        ...
                        isError = shouldValidate && !isProxyCredentialsValid(input),
                        supportingText = { if (shouldValidate && !isProxyCredentialsValid(input)) Text(stringResource(R.string.proxy_auth_invalid)) },
