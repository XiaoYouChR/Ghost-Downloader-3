...
class TaskEditingTest {
    ...
    @Test fun fileSelectionIsIndependentFromEngineSnapshot() {
        ...
        val initial = TaskFilesState(detail, detail.files, setOf(1), setOf(1))
    ...
    @Test fun trimEditsCountAsChanges() {
        val detail = TaskDetail(id = "one", files = listOf(TaskFile(index = 1, size = 10)))
        val state = TaskFilesState(detail, detail.files, setOf(1), setOf(1))
        assertFalse(state.hasChanges)
        val trimmed = state.copy(files = listOf(TaskFile(index = 1, size = 10, startTime = 30, endTime = 90)))
        assertTrue(trimmed.hasChanges)
        assertNull(detail.files.single().startTime)
    }
