...
from .config import (
    m3u8Config,
    resolveM3U8DownloaderExecutable,
    setPreferredM3U8Downloader,
)
...
@dataclass
class M3U8ExtractStage(TaskStage):
    archivePath: str
    installFolder: str
    executablePath: str
...
class M3U8ExtractWorker(Worker):
    chunkSize = 1048576

    def __init__(self, stage: M3U8ExtractStage):
        super().__init__(stage)
        self.stage = stage
    ...
    def _findExecutable(self, root: Path) -> Path | None:
        directPath = root / _executableName("N_m3u8DL-RE")
        if directPath.is_file():
            return directPath

        for candidate in root.rglob(_executableName("N_m3u8DL-RE")):
    ...
    async def _extractTar(self, archivePath: Path, tempDir: Path, totalSize: int):
        ...
        with tarfile.open(archivePath, "r:*") as archive:
            ...
            for member in members:
                ...
                with source, open(targetPath, "wb") as target:
                    while True:
                        chunk = source.read(self.chunkSize)
                        if not chunk:
                            break
    ...
    def _installExtractedFiles(self, extractedRoot: Path, installDir: Path, archiveName: str, tempDirName: str) -> str:
        ...
        executablePath = self._findExecutable(installDir)
        if executablePath is None:
            raise RuntimeError("安装包解压完成，但未找到 N_m3u8DL-RE 可执行文件")

        if sys.platform != "win32":
            executablePath.chmod(executablePath.stat().st_mode | 0o755)

        return _normalizePath(executablePath)

    async def run(self):
        archivePath = Path(self.stage.archivePath)
        if not archivePath.is_file():
            self.stage.setStatus(TaskStatus.FAILED)
            raise FileNotFoundError(f"未找到 N_m3u8DL-RE 安装包: {archivePath}")
        ...
        try:
            ...
            executablePath = self._installExtractedFiles(
                extractedRoot,
                installDir,
                archivePath.name,
                tempDir.name,
            ...
            cfg.set(m3u8Config.installFolder, _normalizePath(installDir))
            setPreferredM3U8Downloader(executablePath)

            if self.stage.cleanupArchive and archivePath.exists():
                archivePath.unlink()

            self.stage.executablePath = executablePath
            self.stage.setStatus(TaskStatus.COMPLETED)
        except asyncio.CancelledError:
            self.stage.setStatus(TaskStatus.PAUSED)
            raise
    ...
    async def run(self):
        ...
        try:
            for stage in self.stages:
                ...
                if isinstance(stage, M3U8ExtractStage):
                    await M3U8ExtractWorker(stage).run()
                    continue
