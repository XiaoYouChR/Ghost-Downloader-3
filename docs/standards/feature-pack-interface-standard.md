...
这份文档定义 Ghost Downloader 后续 Feature Pack 的 V1 标准。
...
- 新代码禁止新增匿名 payload key
- 新代码禁止新增 `_featurePackName`
- 新代码禁止把 Python 类名当作持久化 type id
- 在评审中统一使用 `Task = workflow`、`TaskStage = step`

### Phase 1: 先把名字起对

- 把 `canHandle(url)` 迁到 `accepts(source)`
- 把 `parse(payload)` 迁到 `createTask(data)`
- 把 `canHandleTask(task)` 迁到 `owns(task)`
- 把 `applyPayloadToTask(payload)` 迁到 `configure(config)`
- 把 `syncStagePaths()` 迁到 `syncOutput()`

### Phase 2: 固化 `manifest.toml`

- 固定元数据文件格式
- 固定 Pack 身份字段

### Phase 3: 先建立稳定的作者 API 外壳

- 建立 `app.feature_pack.api`
- 先固定导入路径
- 先暴露最小公共类型

### Phase 4: 抽出 `Task` / `SingleFileTask` / `MultiFileTask`

- 从现有 Pack 中抽公共 Task 行为
- 把 `TaskFile` 正式化
- 让社区作者不再从零写任务骨架
...
- 引入 `TaskConfig`
- 让 `configure()` 成为唯一配置变更入口

### Phase 6: 建立 Actor 风格的信号链路

- 明确 `TaskCard -> Task -> TaskStage` 命令链
- 明确 `TaskStage -> Task -> TaskCard` 事件链
- 统一跨线程 queued connection 约束

### Phase 7: 抽默认卡片和默认测试夹具

- 让社区作者默认不必再写通用 Card 外壳
- 抽出默认 `TaskConfigDialog`
- 抽出默认 `MultiFileSelectDialog`
- 抽出默认 SettingPage 注入能力
- 让社区作者默认不必再写契约测试样板
...
后续任何 Agent 在改动 Feature Pack 相关代码时，必须先问自己六个问题：
...
Ghost Downloader 后续 Feature Pack V1 迁移的关键，不是引入多少新抽象，而是把五件事做好：
...
如果这五件事做好了，V1 就会自然稳定下来，而且社区作者能以较低成本开发新的 Pack。
